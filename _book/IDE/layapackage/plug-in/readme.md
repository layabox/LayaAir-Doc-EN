...
## 18. Custom Resource Thumbnails

By default, the editor generates resource thumbnails automatically. For some resources, developers may want to customize the thumbnail display.

### 18.1 Custom Scene Thumbnails

Scene thumbnails can be customized by implementing the `ISceneThumbnail` interface:

```typescript
@IEditor.regSceneThumbnail()
export class MySceneThumbnail implements IEditor.ISceneThumbnail {
    getThumbnail(scene: IEditor.IGameScene): string | Promise<string> {
        // Return the thumbnail URL
        return "custom-thumbnail.png";
    }
}
```

### 18.2 Custom Asset Thumbnails

For non-scene resources, use the `IAssetThumbnail` interface:

```typescript
@IEditor.regAssetThumbnail()
export class MyAssetThumbnail implements IEditor.IAssetThumbnail {
    getThumbnail(asset: IEditor.IAssetInfo): string | Promise<string> {
        // Return the thumbnail URL based on asset info
        if (asset.ext === "mytype") {
            return "my-thumbnail.png";
        }
        return null;
    }
}
...
## 19. Custom Resource Preview Panel

The resource preview panel allows developers to customize how resources are displayed in the editor.

### 19.1 Basic Usage

```typescript
@IEditor.inspectorLayout("asset")
export class MyPreviewPanel extends IEditor.AssetInspectorLayout {
    constructor() {
        super(MyPreviewType);
    }

    accept(asset: IEditor.IAssetInfo): boolean {
        // Only accept specific resource types
        return asset.ext === "mytype";
    }

    createInspector(asset: IEditor.IAssetInfo, container: gui.Component): any {
        // Create custom preview UI
        let box = new gui.Box();
...
## 20. Custom Resource Import/Export

Developers can customize how resources are imported and exported.

### 20.1 Custom Import

```typescript
@IEditor.regAssetImporter()
export class MyAssetImporter implements IEditor.IAssetImporter {
    accept(asset: IEditor.IAssetInfo): boolean {
        return asset.ext === "mytype";
    }

    async onImport(asset: IEditor.IAssetInfo): Promise<void> {
        // Custom import logic
        console.log("Importing:", asset.url);
    }
}
```
...
```typescript
@IEditor.regAssetExporter()
export class MyAssetExporter implements IEditor.IAssetExporter {
    accept(asset: IEditor.IAssetInfo): boolean {
        return asset.ext === "mytype";
    }

    async onExport(asset: IEditor.IAssetInfo, targetPath: string): Promise<void> {
        // Custom export logic
...
## 21. Custom Resource Configuration Interface

For resources that need special configuration, developers can provide custom configuration interfaces.

### 21.1 Online Resources

Online resources are resources that need to be instantiated in the engine for viewing. Examples include images, sounds, 3D models, etc.
...
@IEditor.inspectorLayout("asset")
export class MyResourceInspector extends IEditor.AssetInspectorLayout {
    constructor() {
        super(MyResourceType);
    }

    accept(asset: IEditor.IAssetInfo): boolean {
        return asset.ext === "myres";
    }

    createInspector(asset: IEditor.IAssetInfo, container: gui.Component): any {
        // Create property inspector
        let inspector = new IEditor.ComponentInspector();
        inspector.target = this;
        container.addChild(inspector);
        return inspector;
    }

    saveInspector(inspector: any, data: any): void {
        // Save resource data
        IEditor.assetDB.saveAsset(asset.url, data);
    }
}
```
...
Resources are saved to files at appropriate times, such as when saving any scene, before building/publishing, before closing the editor, etc.

### 21.2 Offline Resources

Offline resources don't need to be instantiated in the engine for editing or viewing properties. They are typically in JSON/XML/BIN format, used either in the actual game or as configuration in the IDE. Typical examples include bitmap fonts and auto atlases.

Here we also use a custom class, meaning the file content will be a JSON file generated by serializing `DemoABCType`:
...
If the target file is not a JSON file (for example, XML or binary), you can override `FileInspectorLayout`'s `readFile` and `writeFile` methods to return the transformed JSON object.

## 22. Scene Hooks

Scene hooks allow developers to insert custom logic at specific points in the scene lifecycle, such as when creating objects, loading, or saving.
...
### Example: Initialize Anchor Points for All UI Nodes

If you want all newly created UI nodes to have their anchor points initialized to (0.5, 0.5):
